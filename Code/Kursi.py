...
class Kursi:
    ...
    def OK(self):
        ...
        if self.data[0] == 'K1':
            ...
            pass
        global img_bck2
        img_bck2 = ImageTk.PhotoImage(Image.open(resource_path('Stok_Image/back_icon.png')).resize(size=(60, 60)))
    ...
    def chair_K1(self, tiket):
        history = open(resource_path('Data/chair_history_K1.txt'), 'r')
    ...
    def chair_K2(self, tiket):
        history = open(resource_path('Data/chair_history_K2.txt'), 'r')
        h_chair = history.read().split('\n')
        for a in h_chair:
            chair = a.split(';')
            if chair[0] == self.film and chair[1] == self.data[1]:
