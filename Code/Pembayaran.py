...
class Pembayaran:
    ...
    def __init__(self, window, data, makanan='-', main='', home=''):
        ...
        self.dn_icon = ImageTk.PhotoImage(Image.open(resource_path('Stok_Image/dana_icon.jpg')).resize(size=(45,45)))
        self.qr_icon = ImageTk.PhotoImage(Image.open(resource_path('Stok_Image/qris.png')).resize(size=(60,45)))
        self.bri_icon = ImageTk.PhotoImage(Image.open(resource_path('Stok_Image/BRI.png')).resize(size=(50,45)))
    ...
    def gambar_film(self):
        film_file = open(resource_path('Data/film.txt'), 'r').read().split('\n')
        for fg in film_file:
            g = fg.split(',')
            if g[1] == self.film:
                self.lg = g[0]
        ...
        img_open = Image.open(resource_path(self.lg)).resize((250, 380))
        global gambar
    ...
    def qris(self):
        ...
        self.qris_ic = ImageTk.PhotoImage(Image.open(resource_path('Stok_Image/qris.png')).resize(size=(130,50)))
        ...
        self.qr_gen.save(resource_path("Stok_Image/payment_code.png"))
        self.qr_code = ImageTk.PhotoImage(Image.open(resource_path('Stok_Image/payment_code.png')).resize(size=(300,300)))
    ...
    def save(self):
        l = self.data[3]
        h = open(resource_path(f'Data/chair_history_{self.data[1]}.txt'), 'r')
        c = h.read().split('\n')
        x = open(resource_path(f'Data/chair_history_{self.data[1]}.txt'), 'w')
        p = 0
        for a in c:
            if a == '':
                continue
