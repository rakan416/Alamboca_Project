...
class Makanan:
    ...
    def __init__(self,window,  data, main, home):
        ...
        self.sh = CTkButton(self.fr_makanan, text='COMBO', font=('Times New Roman', 15, 'bold'), fg_color='#f5f0e1', command=lambda: self.slide(0, self.sh), bg_color='#ff6e40', text_color='black', height=35, corner_radius=20)
        self.sh.grid(column=0, row=0, padx=10, pady=7)
        self.fi = CTkButton(self.fr_makanan, text='PROMO COMBO', font=('Times New Roman', 15, 'bold'), fg_color='#ffc13b', command=lambda: self.slide(-1100, self.fi), bg_color='#ff6e40', text_color='black', height=35, corner_radius=20)
        self.fi.grid(column=1, row=0, padx=10, pady=7)
        self.hr = CTkButton(self.fr_makanan, text='POPCORN', font=('Times New Roman', 15, 'bold'), fg_color='#ffc13b', command=lambda: self.slide(-2200, self.hr), bg_color='#ff6e40', text_color='black', height=35, corner_radius=20)
        self.hr.grid(column=2, row=0, padx=10, pady=7)
        self.an = CTkButton(self.fr_makanan, text='MINUMAN', font=('Times New Roman', 15, 'bold'), fg_color='#ffc13b',command=lambda: self.slide(-1100*3, self.an), bg_color='#ff6e40', text_color='black', height=35, corner_radius=20)
        self.an.grid(column=3, row=0, padx=10, pady=7)
        self.km = CTkButton(self.fr_makanan, text='MAKANAN', font=('Times New Roman', 15, 'bold'), fg_color='#ffc13b',command=lambda: self.slide(-1100*4, self.km), bg_color='#ff6e40', text_color='black', height=35, corner_radius=20)
        self.km.grid(column=4, row=0, padx=10, pady=7)
        self.data=open(resource_path('Data/makanan.txt'), "r").read().split('\n')
        self.i = 0
        ...
        global icon_pay
        icon_pay = ImageTk.PhotoImage(Image.open(resource_path('Stok_Image/PAY.png')).resize(size=(50, 50)))
        self.paybtt = Button(self.FRAME, image=icon_pay, bg='#1e3d59', command=self.send, border=0, relief=FLAT)
        self.paybtt.place(x=1030, y=40)

        global img_bck
        img_bck = ImageTk.PhotoImage(Image.open(resource_path('Stok_Image/back_icon.png')).resize(size=(60, 60)))
    ...
    def slide(self, stop, btt):
        ...
        self.norm()
        btt.configure(fg_color='#f5f0e1')

    def norm(self):
        self.sh.configure(fg_color='#ffc13b')
        self.fi.configure(fg_color='#ffc13b')
        self.hr.configure(fg_color='#ffc13b')
        self.an.configure(fg_color='#ffc13b')
        self.km.configure(fg_color='#ffc13b')
    ...
    def btt_loop(self,frame,  promo):
        ...
        y = 10
        def img_pro(file):
            img_open = Image.open(resource_path(file)).resize((150, 150))
            global gambar
...
# Tampilan = Tk()
# Makanan(Tampilan, data='-', main='-', home='-')
# Tampilan.mainloop() 